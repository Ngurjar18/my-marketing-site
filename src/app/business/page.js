...
export default function OurBusiness() {
  return (
    <div className="min-h-screen bg-gray-100 text-gray-900 overflow-x-hidden">
      <Navbar />
      <h1 className="text-5xl font-extrabold text-center text-navy py-16 px-6 mt-24">
        Our Businesses
      </h1>

      <div className="flex flex-wrap justify-center gap-6 px-4 mt-1 mb-20 w-full">
        {businesses.map((business) => (
          <div
            key={business.id}
            className="cursor-pointer bg-white shadow-lg p-4 rounded-lg transition-transform transform hover:scale-105 text-center w-[90%] sm:w-[350px] h-[350px] flex flex-col items-center justify-center"
            onClick={() =>
      ...
      <div className="space-y-16 px-4 w-full">
        {businesses.map((business) => (
          <div key={business.id} id={business.id} className="scroll-mt-24 bg-white p-6 rounded-lg shadow-lg w-full">
            <h2 className="text-3xl md:text-5xl font-bold text-navy mb-8 text-center">{business.title}</h2>

            {business.sections.map((section, i) => (
              <div key={i} className={`flex flex-col md:flex-row ${i % 2 === 0 ? "md:flex-row-reverse" : ""} items-center gap-6 mb-6`}>
                <Image src={section.image} alt={business.title} width={400} height={300} unoptimized className="rounded-lg w-full md:w-1/2" />

                <div className="flex-1 text-lg text-gray-700 w-full md:w-1/2">
      ...
      {/* Footer */}
      <footer className="py-6 text-center bg-navy text-white shadow-inner w-full">
